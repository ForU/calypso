...
class ModelIface(object):
    ...
    def __setattr__(self, k, v):
        ...
        if isinstance(v, Field):
            ...
            return
        # assignment
        is_fld, fv = self._is_field_registed(k)
        # only apply to Field attributes
        if is_fld:
            try:
                fv.value = fv.type(v)
            except Exception as e:
    ...
    def dumpAsDict(self,):
        return { k:v.value for k,v in self.__dict__.items() if isinstance(v, Field) }
    ...
    def dumpAsDBdata(self,):
        ...
        return { str(k):str(v.value if v.value else v.default) # v.value or v.default
...
class Join(TableIface):
    ...
    def _set_dynamic_fields(self, mc_l, mc_r):
        ...
        self._jn_dynamic_fields = { v.name:v.definition() for k, v in dict_fields.items() }
